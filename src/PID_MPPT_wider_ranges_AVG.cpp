...
void setup(void)
{
  ...
  atverterE.setLED(LED1G_PIN, LOW);
}
...
void controlUpdate(void)
{
  ...
  // -----------------------------------------------------------------------------------------------------------------------------------
/*  
  if(avgCount == NUM_AVERAGES) {    // perform full IC operation

    // calculate current averages
    avgLowCurrent = avgLowCurrentSum / NUM_AVERAGES;
    avgLowVoltage = avgLowVoltageSum / NUM_AVERAGES;
    ...
    if((-VOLTAGE_ERROR_RANGE < dV) && (dV < VOLTAGE_ERROR_RANGE)) {
        ...
        if((dI/dV > -avgLowCurrent/avgLowVoltage)) {
  ...
  else {    // store data for avg calculation
